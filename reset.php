...
if(isset($_POST['submit'])){
    ...
    if($errorMessage != ''){
        ...
        try{
            ...
            $sql = 'SELECT email, resetpassword FROM accounts WHERE email = :email';
            $s = $conn->prepare($sql);
        ...
        if($count < 1){
            ...
            $temppass = $row['resetpassword'];

            try{
                $sql2 = 'SELECT * FROM accounts WHERE email = :email AND resetpassword = :temppass';
